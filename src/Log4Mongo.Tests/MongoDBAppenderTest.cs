...
")));
        ...
        public void Should_log_timestamp()
        {
            ...
            var doc = _collection.Find<BsonDocument>(new BsonDocument());
            var log = doc.FirstOrDefault();
        ...
        [Test]
        public void Should_create_expiry_index()
        {
            var logRepository = LogManager.GetRepository(Assembly.GetEntryAssembly());
            XmlConfigurator.Configure(logRepository, new MemoryStream(Encoding.UTF8.GetBytes(@"
            <log4net>
        ...
        ")));
            ...
            var c = _collection.Count(new BsonDocument());
            c.Should().Be.EqualTo(0);

            target.Info(6);
            _collection.Find<BsonDocument>(new BsonDocument())
                .ToList()
                .Select(x => x.GetElement("message").Value.AsString)
                .Should()
